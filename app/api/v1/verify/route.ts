...
const generateMockPayload = (serviceType: string, requestData: any) => {
	const isKRA =
		serviceType.toLowerCase().includes("kra") ||
		serviceType.toLowerCase().includes("pin");
	const isAML = serviceType.toLowerCase().includes("aml");

	if (isKRA) {
		return {
			pin: requestData.pin || "P000000000X",
			taxpayerName: requestData.firstName
...
export async function POST(req: Request) {
	try {
		...
		if (!userId || !serviceType || !entityData) {
			return NextResponse.json(
				{ error: "Missing required fields (userId, serviceType, entityData)" },
				{ status: 400 },
			);
		}
		...
		try {
			...
			return NextResponse.json(
				{ error: error.message || "Balance deduction failed" },
				{ status: 402 },
