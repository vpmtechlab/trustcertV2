...
export function WeeklyComparisonChart() {
  ...
  const chartData = useQuery(api.analytics.getWeeklyComparison, 
    member?.companyId ? { companyId: member.companyId as Id<"companies"> } : "skip"
  );

  const thisWeekTotal = chartData?.reduce((sum, d) => sum + d.current, 0) || 0;
  const lastWeekTotal = chartData?.reduce((sum, d) => sum + d.previous, 0) || 0;
